...
const Ticket = (props) => { 
   ...
   {props.id && sessionStorage.setItem("ticketId", props.id);}
    const id= props.id ? props.id : sessionStorage.getItem("ticketId");
    ...
    if(!id) {return(<>
        ...
        <nav className="flex gap-14 ml-[70%] mt-3 text-[13px] md:gap-10 md:ml-[73%] md:mt-[-3.5%] md:text-[10px] lg:ga-12 lg:ml-[75%] lg:mt-[-3.4%] lg:text-[12px] xl:gap-14 xl:ml-[78%] xl:mt-[-2%] xl:text-[15px] 2xl:gap-16 2xl:ml-[79%] 2xl:mt-[-2%] 2xl:text-[15px] " >
            <a href="http://localhost:5177/homepage">Home</a>
            <a href="http://localhost:5177/ticket">Tickets</a>
            <a href="http://localhost:5177/homepage">Profile</a>
        </nav>
    </header>
    <h1 className="font-bold mt-5 text-3xl ml-[450px]">Ticket Not Available ...</h1>
    </>
) }
    return (
        <>  
